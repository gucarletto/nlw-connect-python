...
event_route_bp = Blueprint('suscriptions_route', __name__)
...
@event_route_bp.route('/subscription', methods=['POST'])
def create_new_subscription():
  subscribers_creator_validator(request.json)
  http_request = HttpRequest(body=request.json)

  events_creator = SubscribersCreator(SubscribersRepository())
  http_response = events_creator.create_subscription(http_request)
